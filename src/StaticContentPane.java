...
public class StaticContentPane extends JPanel implements MegaTConstants {
	...
	public StaticContentPane() {
		...
		JComboBox cbx_templName = new JComboBox(new String[] { "Virselis",
				"Aplankas", "Lakas" });
		...
		JLabel lblPressSheet = new JLabel("Press sheet size:",
				SwingConstants.RIGHT);
		txf_pressSheet = new JTextField(4);
		JLabel lblWS = new JLabel("Work style:", SwingConstants.RIGHT);
		cbx_WS = new JComboBox(new String[] { "Sheetwise", "Work and Turn",
		...
		gbc_paneTemplTree.insets = new Insets(TOP_TOP, LEFT, BOTTOM_BOTTOM,
				RIGHT_RIGHT);
		...
		gbc_paneTemplLayout.insets = new Insets(TOP, LEFT_LEFT, BOTTOM_BOTTOM,
				RIGHT);
	...
	public void setWorkstyle(Signature sign) {

		cbx_WS.setSelectedIndex(sign.getWorkStyle());
	}

	/**
	 * Shows width and height of the press sheet
	 * @param sign
	 */
	public void setPressSheetSize(Signature sign) {

		String w = convertToString(sign.getSignWidth());
		String h = convertToString(sign.getSignHeight());
		String fieldValue;
	...
	/**
	 * Shows height of the page
	 * @param value
	 */
	public void setPageHeight(Signature sign) {

		String fieldValue = convertToString(sign.getPageHeight());
		txf_pageHeight.setText(fieldValue);
	}


	/**
	 * Converter of measurment units
	 * @param value
	 * @return
	 */
	private static String convertToString(float value) {
		...
		if (MEASURMENT.equals("mm")) {
			...
			value = Utility.pointsToMillimeters(value);

		} else if (MEASURMENT.equals("in")) {

			value = Utility.pointsToInches(value);

		} else if (MEASURMENT.equals("cm")) {

			value = Utility.pointsToMillimeters(value) / 10;
		}
